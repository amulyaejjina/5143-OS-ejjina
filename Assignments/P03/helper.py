...
class Reader(threading.Thread):
    ...
    def run(self,layout):
        ...
        for each in self.__to_write:
            ...
            readvalue1 = self.__l.readfrommemory(read1.split()[1])
            readvalue2 = self.__l.readfrommemory(read2.split()[1])
            rownum1 = rownum1 + 1
            if (rownum1 == 20):
                blah1 = ""
                rownum1 = 0
            if read1.split()[2] == 'R1':
                ...
                layout["body"].update(Panel(blah1, border_style="#00FF00"))
...
class Writer(threading.Thread):
    ...
    def run(self,layout):
        ...
        for each in self.__to_write:
            ...
            layout["side"].update(Panel(blah, border_style="#00FF00"))
            ...
            if action.split()[0] == 'DIV':
                self.__registers = alu.exec("add")
            #print(self.__registers[0])
            self.__l.updatelocalmemory(write.split()[2],self.__registers[0])
            rownum = rownum + 1
            if (rownum == 40):
                blah = ""
                rownum = 0
        ...
        #return self.__l
            ...
            time.sleep(random.random())
        return self.__l
        # for i in range(len(self.__to_write)):
        #     print(self.__to_write[i])
            # for j in range(len(self.__to_write[i])):
            #     print(self.__to_write[i][j].split())
